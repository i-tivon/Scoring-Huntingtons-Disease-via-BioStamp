function [selected_fts, selected_test_fts, fLabels] = selectFeats(allTrainFeats, allTestFeats,...
    train_labels, labelset, taskList, type, isbinary ,useLasso)
if useLasso
    disp('using Lasso');
        flg = 0;
    ...
    %         [B_gait, fitInfo] = lassoglm(allTrainFeats,train_labels,'normal','CV',length(train_labels)-1);
            disp('normal Lasso')
            [B_gait, fitInfo] = lassoglm(allTrainFeats,train_labels,'normal','CV',length(train_labels)-1);
%             [idx,scores] = fscmrmr(allTrainFeats,train_labels,'Prior','empirical');
%             i_sft = idx(1:10)';
            %flg =1;
            % TODO add option for classification of scores
        end
    ...
    disp('not using lasso');

    i_sft = 1:size(allTrainFeats,2);
end
